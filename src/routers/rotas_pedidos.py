...
def exibir_pedido(id:int,session:Session = Depends(get_db)):
    try:
        ...
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail=f'Não existe pedido com id={id}')
...
@router.get('/pedidos_vendas',response_model=List[Pedido])
def listar_vendas(usuario:Usuario = Depends(obter_usuario_logado),session:Session = Depends(get_db)):
    pedidos = RepositorioPedido(session).listar_minhas_vendas_por_usuario_id(usuario.id)
    return pedidos